...
@router.get("/eligible-companies")
async def get_eligible_companies(
    billing_month: str,
    current_user: dict = Depends(get_current_user)
):
    """
    Get list of companies eligible for invoice generation for a specific billing month.
    Only returns companies whose start_date is on or before the billing period.
    """
    check_permission(current_user, "view_invoice")
    ...
    for company in companies:
        ...
        company_info = {
            ...
            "company_name": company["company_name"],
            "start_date": start_date_str,
            "total_rate": company.get("total_rate", 0),
        ...
        if start_date_str:
            try:
                ...
                if start_date > billing_date:
                    company_info["reason"] = f"Start date ({start_date_str}) is after billing period ({billing_month})"
                    ineligible.append(company_info)
                    continue
            except ValueError:
                pass
        ...
        if existing:
            company_info["reason"] = f"Invoice already exists for {billing_month}"
            company_info["existing_invoice_number"] = existing.get("invoice_number")
            ineligible.append(company_info)
            continue
        
        eligible.append(company_info)
    
    # Calculate due date (4 days after invoice date)
    due_date = billing_date + timedelta(days=4)
